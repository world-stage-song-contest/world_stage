...
-- Audit log for song changes, populated by a trigger on the song table.

CREATE TABLE song_audit_log (
    ...
    -- song_replacement / update: dict of {field: {old, new}}
    changed_fields jsonb
);

CREATE INDEX song_audit_log_changed_at_idx ON song_audit_log (changed_at DESC);
CREATE INDEX song_audit_log_song_id_idx    ON song_audit_log (song_id);

-- Trigger-based audit logging for the song table.
-- The trigger reads the current user from the session variable
-- app.current_user_id (set via set_config() in the application layer).
...
BEGIN
    ...
    IF TG_OP = 'UPDATE' THEN
        ...
        IF v_changed != '{}' THEN
            ...
            -- Log remaining field changes as song_change
            IF (v_changed - CASE WHEN v_song_changed THEN 'title' ELSE '' END
                           - CASE WHEN v_song_changed THEN 'artist' ELSE '' END
                           - CASE WHEN v_placeholder_changed THEN 'is_placeholder' ELSE '' END) != '{}' THEN
                INSERT INTO song_audit_log
                    (song_id, event_type, changed_by,
                     song_title, song_artist, song_country_id, song_year_id,
                     changed_fields)
