...
-- Add optional year-range filtering (and, for submitter variants, an
-- include-specials toggle) to the bias functions. Parameter lists
-- changed, so the old definitions must be dropped before the new ones
-- are created.

DROP FUNCTION IF EXISTS user_country_bias(bigint);
DROP FUNCTION IF EXISTS user_submitter_bias(bigint);
DROP FUNCTION IF EXISTS country_voter_bias(text);
DROP FUNCTION IF EXISTS submitter_voter_bias(bigint);
