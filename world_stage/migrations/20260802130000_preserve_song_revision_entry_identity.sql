-- A move changes the stable song's current slot, not the historical identity
-- of every content revision.  New revisions receive the current slot from
-- set_song_data_entry_tuple; existing revisions must retain their own tuple.
DROP TRIGGER IF EXISTS sync_song_data_entry_tuple ON song;
DROP FUNCTION IF EXISTS sync_song_data_entry_tuple();
