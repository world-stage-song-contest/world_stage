...
CREATE TABLE IF NOT EXISTS saved_search_query (
...
CREATE TABLE IF NOT EXISTS year_spot_watch (
    account_id bigint NOT NULL
        REFERENCES account (id) ON UPDATE RESTRICT ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS year_spot_watch_spot_idx
...
CREATE TABLE IF NOT EXISTS genre_set (
    id bigint PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY,
    subgenre_ids bigint[] NOT NULL UNIQUE,
    CHECK (cardinality(subgenre_ids) > 0)
);

CREATE TABLE IF NOT EXISTS genre_set_subgenre (
    genre_set_id bigint NOT NULL
        REFERENCES genre_set (id) ON UPDATE RESTRICT ON DELETE CASCADE,
    subgenre_id bigint NOT NULL
        REFERENCES subgenre (id) ON UPDATE RESTRICT ON DELETE RESTRICT,
    priority smallint NOT NULL,
    PRIMARY KEY (genre_set_id, priority),
    UNIQUE (genre_set_id, subgenre_id)
);

CREATE TABLE IF NOT EXISTS key_signature_set (
    id bigint PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY,
    signatures jsonb NOT NULL UNIQUE,
    CHECK (jsonb_typeof(signatures) = 'array' AND jsonb_array_length(signatures) > 0)
);

CREATE TABLE IF NOT EXISTS key_signature_set_key_signature (
    key_signature_set_id bigint NOT NULL
        REFERENCES key_signature_set (id) ON UPDATE RESTRICT ON DELETE CASCADE,
    priority smallint NOT NULL,
...
CREATE TABLE IF NOT EXISTS time_signature_set (
    id bigint PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY,
    signatures jsonb NOT NULL UNIQUE,
    CHECK (jsonb_typeof(signatures) = 'array' AND jsonb_array_length(signatures) > 0)
);

CREATE TABLE IF NOT EXISTS time_signature_set_time_signature (
    time_signature_set_id bigint NOT NULL
        REFERENCES time_signature_set (id) ON UPDATE RESTRICT ON DELETE CASCADE,
    priority smallint NOT NULL,
    start_seconds integer NOT NULL DEFAULT 0 CHECK (start_seconds >= 0),
    numerator smallint,
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_artist_name_number_ci
    ON artist (LOWER(full_name), number);

CREATE TABLE IF NOT EXISTS artist_credit_set (
    id bigint PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY
);
...
CREATE TABLE IF NOT EXISTS artist_credit (
    ...
    stage_name text CHECK (stage_name IS NULL OR stage_name ~ '[^[:space:]]'),
    join_phrase text,
...
CREATE INDEX IF NOT EXISTS idx_artist_credit_artist ON artist_credit (artist_id);
...
CREATE TABLE IF NOT EXISTS song_data (
    ...
    genre_set_id bigint
        REFERENCES genre_set (id) ON UPDATE RESTRICT ON DELETE RESTRICT,
    key_signature_set_id bigint
        REFERENCES key_signature_set (id) ON UPDATE RESTRICT ON DELETE RESTRICT,
    time_signature_set_id bigint
        REFERENCES time_signature_set (id) ON UPDATE RESTRICT ON DELETE RESTRICT,
    submitter_id bigint REFERENCES account (id) ON UPDATE RESTRICT ON DELETE RESTRICT,
    title text,
    artist_credit_set_id bigint
...
CREATE INDEX IF NOT EXISTS idx_song_data_genre_set
    ON song_data (genre_set_id);
CREATE INDEX IF NOT EXISTS idx_song_data_key_signature_set
    ON song_data (key_signature_set_id);
CREATE INDEX IF NOT EXISTS idx_song_data_time_signature_set
    ON song_data (time_signature_set_id);
...
CREATE TABLE IF NOT EXISTS point_system (
    id bigint PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY,
    kind text NOT NULL DEFAULT 'ranked' CHECK (kind IN ('ranked', 'pool')),
    metadata jsonb NOT NULL DEFAULT '{"points": []}',
    ...
    required_items integer GENERATED ALWAYS AS (
        CASE WHEN kind = 'pool' THEN point_pool_required_items(metadata)
        ELSE jsonb_array_length(metadata->'points') END
    ) STORED,
    CONSTRAINT point_system_metadata_valid CHECK (valid_point_system_metadata(kind, metadata))
);
...
CREATE TABLE IF NOT EXISTS admin_saved_query (
    id bigint PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY,
    name text NOT NULL,
...
CREATE UNIQUE INDEX IF NOT EXISTS admin_saved_query_name_unique
...
CREATE TABLE IF NOT EXISTS admin_query_log (
    ...
    operation text NOT NULL CHECK (operation IN ('select', 'insert', 'update', 'delete', 'other')),
...
CREATE INDEX IF NOT EXISTS admin_query_log_created_at_idx
    ON admin_query_log (created_at DESC);

CREATE INDEX IF NOT EXISTS admin_query_log_fingerprint_idx
    ON admin_query_log (query_fingerprint, created_at DESC);

CREATE INDEX IF NOT EXISTS admin_query_log_actor_idx
...
CREATE INDEX IF NOT EXISTS song_play_user_played_at_idx
    ON song_play (user_id, played_at DESC, id DESC);
...
CREATE INDEX IF NOT EXISTS password_reset_token_account_idx
...
-- Materialize the complete ballot-entry policy once for result refreshes.
-- The scalar ballot_entry_rule remains the authority for validating a single
-- form entry; this set-based equivalent avoids repeating its show, point, and
-- ownership lookups for every cell of a result matrix.
...
LANGUAGE sql STABLE AS $$
WITH point_max AS (
    SELECT id AS point_system_id, max_score FROM point_system
),
user_shows AS (
