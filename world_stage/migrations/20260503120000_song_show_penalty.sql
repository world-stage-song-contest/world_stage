...
-- Penalty points applied to a song in a specific show — typically used
-- to dock the maximum possible score from a song whose submitter failed
-- to vote in that show. Defaults to 0 and is subtracted from
-- ``country_show_results.total_points`` by ``refresh_show_results``.

ALTER TABLE song_show ADD COLUMN IF NOT EXISTS penalty integer NOT NULL DEFAULT 0;

-- Replace the show-results function with a penalty-aware version.
...
-- Trigger refresh when a penalty is set/unset.
