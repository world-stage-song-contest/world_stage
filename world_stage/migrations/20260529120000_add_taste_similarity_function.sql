...
-- "Users with the most similar tastes": for a target voter, rank every other
-- voter by how closely their points agree, over the shows both took part in.
--
-- Metric: a within-show Pearson correlation, pooled across shared shows.
--   * Comparison basis per show = songs BOTH voters ranked (gave points to).
--     Restricting to the intersection keeps the zero point meaningful: two
--     unrelated voters' scores on their shared songs are ~uncorrelated (≈ 0),
--     genuine agreement pushes toward +1, genuine disagreement toward -1. The
--     wider "union" basis was rejected because one-sided 0s (ranked by one,
--     ignored by the other) inject structural anti-correlation that drags the
--     whole scale negative, so no pair ever reads decently positive.
--   * Songs submitted by either of the two voters are dropped — you can't
--     score your own entry, so it would skew the pair.
--   * Each show is centred on its own mean before pooling. Points are a fixed
--     pool per show (12/10/8/...), so centring cancels the point-system/scale
--     differences between shows and turns the pooled coefficient into an
--     honest correlation.
--   * Voters with fewer than 5 shared shows (same threshold the bias functions
--     use) are flagged 'inconclusive' rather than dropped, so the page can show
--     them but offer a "hide inconclusive" toggle.
--
-- One target user vs everyone in a single pass (no N^2 fan-out). Sort the
-- result descending for kindred spirits, ascending for contrarians.

-- Dropped + recreated (not CREATE OR REPLACE): the RETURNS TABLE shape changed.
